...
- `GET /v1/models` (`:215`) → the public virtual IDs `myownllm` (chat), `myownllm-transcribe` (ASR surface), `myownllm-speak` (TTS surface), and `myownllm-embed` (embeddings surface), each with `resolved_to`, plus every raw pulled Ollama tag. (`PUBLIC_VIRTUAL_IDS`, resolver.rs.)
- `POST /v1/embeddings` → translates `myownllm-embed` to the hardware-appropriate Ollama embedding model (EmbeddingGemma / Nomic / all-MiniLM), pulls-on-demand (honours `?wait=true` / `X-MyOwnLLM-Wait`), proxies to Ollama. `embed` is a **tracked** mode in MyOwnLLM, so the engine keeps a local embedding model pulled + warm — this is the surface Myo's native memory system embeds against (`myo-core` `LlmClient::embed` → `myo_embed` command).
